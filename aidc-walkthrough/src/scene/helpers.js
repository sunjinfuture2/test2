...
/* ══ 실측 스케일 ═══════════════════════════════════════════
   원본(용어사전 부감 뷰)은 층이 트여 보이도록 층고를 1.5배 늘리고
   씬 루트에 수평 1.8 · 수직 2.2배를 더 걸었다. 부감으로 볼 때는
   보기 좋지만, 1인칭으로 들어가면 눈높이 1.65 m 대비 천장이 26배가 되어
   즉시 가짜로 보인다. 워크스루는 도면 좌표를 그대로 미터로 쓴다.

   결과: 층고 13.5 m · 벽 높이 12.5 m · 슬래브 1 m — 실제 치수. */
export const FLOOR_SCALE = 1

const SRC_LV = [0, 13.5, 27, 40.5]   // B1 · 1F · 2F · 옥상 (m)
export const PITCH = 13.5
/** 층 바닥 높이 [B1, 1F, 2F, 옥상] (m) */
export const LEVELS = SRC_LV.slice()

/** 장비 높이 — 실측이므로 항등 */
export function MZ(z) { return z }

/** 구조체(벽·슬래브) 높이 — 실측이므로 항등 */
export function MZS(z) { return z }
